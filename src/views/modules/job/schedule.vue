  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$httpk({
          url: this.$httpk.adornUrl('/sys/schedule/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$httpk({
            url: this.$httpk.adornUrl('/sys/schedule/delete'),
            method: 'post',
            data: this.$httpk.adornData(ids, false)
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
      ...
      pauseHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '暂停' : '批量暂停'}]操作?`, '提示', {
          ...
          this.$httpk({
            url: this.$httpk.adornUrl('/sys/schedule/pause'),
            method: 'post',
            data: this.$httpk.adornData(ids, false)
          }).then(({data}) => {
            if (data && data.code === 0) {
      ...
      resumeHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '恢复' : '批量恢复'}]操作?`, '提示', {
          ...
          this.$httpk({
            url: this.$httpk.adornUrl('/sys/schedule/resume'),
            method: 'post',
            data: this.$httpk.adornData(ids, false)
          }).then(({data}) => {
            if (data && data.code === 0) {
      ...
      runHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '立即执行' : '批量立即执行'}]操作?`, '提示', {
          ...
          this.$httpk({
            url: this.$httpk.adornUrl('/sys/schedule/run'),
            method: 'post',
            data: this.$httpk.adornData(ids, false)
          }).then(({data}) => {
            if (data && data.code === 0) {
