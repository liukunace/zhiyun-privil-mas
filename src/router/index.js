...
const globalRoutes = [
  ...
  {
    path: '/oauth2loginxxxxX6ascoajcoajciashdcias',
    name: 'login',
...
const mainRoutes = {
  ...
  children: [
    ...
    // { path: '/projects', component: _import('basiness-views/projects'), name: 'demo-ueditor', meta: { title: '项目管理视图', isTab: true } }
  ],
  beforeEnter(to, from, next) {
    let token = Vue.cookie.get('token')
    if (!token || !/\S/.test(token)) {
      console.log("token 失效 --- router.js--line 53");
    }
    next()
...
router.beforeEach((to, from, next) => {
  let token2 = Vue.cookie.get('token');
  let per2 = sessionStorage.getItem('permissions');
  let tokendata = token.loadToken().access_token;
  let username = Vue.cookie.get("username");
  let cookiedata = Vue.cookie.get(username);
  if (tokendata == "null" || tokendata == null) {
